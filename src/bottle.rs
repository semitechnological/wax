...
impl BottleDownloader {
    ...
    fn relocate_macho(path: &Path, prefix: &str, cellar: &str, library: &str) -> Result<()> {
        ...
        if fix_macho_install_name(path_str, path, prefix, cellar, library) {
            modified = true;
        }

        if fix_macho_loaded_libraries(path_str, path, prefix, cellar, library) {
            modified = true;
        }

        if fix_macho_rpaths(path_str, path, prefix, cellar, library) {
            modified = true;
        }
        ...
        .output()
    {
        if output.status.success() {
            ...
            for lib_path in macho_loaded_libraries(&text) {
                ...
                let result = std::process::Command::new("install_name_tool")
