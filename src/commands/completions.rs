...
fn generate_shell_completions(shell: Shell, home: &str) -> Result<(PathBuf, Vec<u8>)> {
    match shell {
        Shell::Zsh => {
...
fn install_completions(shell: Shell) -> Result<()> {
    ...
    let (dest, content) = generate_shell_completions(shell, &home)?;
